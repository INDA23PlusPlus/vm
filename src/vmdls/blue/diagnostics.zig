...
/// Produce diagnostics for Blue source.
pub fn produceDiagnostics(doc: *Document, alloc: std.mem.Allocator) !void {
    ...
    var success = true;
    var comp = blue.compile(doc.text, alloc, &diagnostics, true, null) catch blk: {
        success = false;
        break :blk undefined;
    };
    defer if (success) comp.deinit();

    var msg_buf = std.ArrayList(u8).init(alloc);
    defer msg_buf.deinit();

    std.log.info(
        "{d} diagnostics found (exluding related information)",
